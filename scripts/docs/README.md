...
> **Self-architecture role**: Layer ③ Automation per [`STRATEGY.md §10`](../../template/docs/STRATEGY.md) and [ADR-0007](../../template/docs/01-artefacts/arc42/09-decisions/0007-automation-layer-contract.md). Outputs are Layer ⑤ Measurement artefacts (placed under `05-measurement/` of the target project).
>
> **Layer-writes contract** (per ADR-0007): these scripts **read** layers ⓪-④ and **write** only to Layer ⑤ (`05-measurement/`) or CI logs. They do not mutate any other layer.

## Scripts

| Script | Reads | Writes | Purpose |
| --- | --- | --- | --- |
| [`metrics_coverage.py`](./metrics_coverage.py) | `01-artefacts/arc42/`, `01-artefacts/detailed-design/`, `01-artefacts/arc42/03-context-and-scope/use-cases/` (Layer ①) | stdout (json / markdown) | Existence + substantive-content coverage per arc42 section |
...
| [`metrics_adr.py`](./metrics_adr.py) | `01-artefacts/arc42/09-decisions/*.md` (Layer ①) | stdout (json / markdown) | ADR Status distribution + monthly throughput |
...
JSON schema is documented in each script's `--help`. Markdown output is suitable for committing to `05-measurement/snapshots/YYYY-MM-DD_*.md` for trend tracking.
...
- **Dogfoodable**: the kit runs these scripts against its own `template/docs/` to produce [`../../template/docs/05-measurement/baseline.md`](../../template/docs/05-measurement/baseline.md).

## Cross-references

- [STRATEGY.md §12 Layer ⑤ Measurement](../../template/docs/STRATEGY.md)
- [ADR-0009 Measurement layer activation](../../template/docs/01-artefacts/arc42/09-decisions/0009-measurement-layer-activation.md)
- [`../../template/docs/05-measurement/README.md`](../../template/docs/05-measurement/README.md) — Layer ⑤ artefact navigation
