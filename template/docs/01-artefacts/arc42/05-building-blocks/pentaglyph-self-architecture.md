...
C4Context
    ...
    Rel(reader, pentaglyph, "Reads via downstream project's 01-artefacts/user-manual/", "indirect")
...
C4Container
    ...
    Container_Boundary(kit, "pentaglyph kit") {
        Container(measurement, "⑤ Measurement (optional)", "scripts/docs/metrics_*, 05-measurement/", "Quantifies health of layers ⓪-④. Not required for v1 of pentaglyph adoption.")
        Container(governance, "④ Governance", "04-governance/", "RACI, ADR Accept protocol, override justification, contribution guide")
        Container(automation, "③ Automation", "cli/, .claude/, scripts/docs/", "Bun CLI scaffolder + AI agents + lint/coverage/sync scripts")
        Container(process, "② Process", "02-process/", "Bindings to external process canons: Scrum, BDD, TDD, Trunk-based, …")
        Container(artefacts, "① Artefacts", "01-artefacts/templates/, STRATEGY.md, WORKFLOW.md, AI_INSTRUCTIONS.md", "9+ templates + placement taxonomy + lifecycle state machine")
...
| ① Artefacts | `01-artefacts/templates/`, `STRATEGY.md`, `WORKFLOW.md`, `AI_INSTRUCTIONS.md`, plus every directory under `01-artefacts/arc42/`, `01-artefacts/detailed-design/`, `01-artefacts/api-contract/`, `01-artefacts/user-manual/`, `01-artefacts/service-design/`, `01-artefacts/diagrams/c4/` | Concrete shapes + placement + lifecycle | ② |
| ② Process | `02-process/` (one file per bound canon + `_binding-a-new-process.md` meta-doc) | Thin operational defaults bound to external process canons | ③ |
| ③ Automation | `cli/`, `.claude/`, `scripts/docs/` | Execute ② and operate on ① | ④ |
| ④ Governance | `04-governance/` | Who decides, accepts, overrides | ⑤ |
| ⑤ Measurement *(optional)* | `05-measurement/`, `scripts/docs/metrics_*` | Quantify ⓪-④ health | — |
...
3. **Layer ② design-guides may reference Layer ① templates** (e.g. "BDD output uses `01-artefacts/templates/2_prd.md` Acceptance Criteria section") but never Layer ③ automation.
...
| ① Artefacts | High | Replace `01-artefacts/templates/` and `STRATEGY.md §3+`; usually means forking the kit | Switching from MADR to Y-Statement-only ADRs. **Rare.** |
| ② Process | Medium | Replace specific files under `02-process/`; keep ⓪-① intact | Replacing Scrum with Kanban; replacing Git Flow with Trunk-Based. **Common.** |
| ③ Automation | Low | Disable / replace CLI commands or `.claude/` rules | Using Bazel instead of Bun for scaffolding. **Project-specific.** |
| ④ Governance | Low | Replace specific files under `04-governance/` | Stronger ADR Accept requirements for regulated industries. **Common.** |
| ⑤ Measurement | None | Skip entirely (optional layer) | Small projects that don't need dashboards. **Common.** |

Each downstream override must include a one-paragraph rationale in [`docs/02-process/`](../../02-process/) and **link back to the layer + this file**, so the deviation is greppable and reversible.
...
- [`../../01-artefacts/diagrams/c4/workspace.dsl`](../../01-artefacts/diagrams/c4/workspace.dsl) — Structurizr DSL (downstream projects). The kit's own C4 lives in the Mermaid blocks above instead, because the kit is small enough not to warrant a full DSL workspace.
