...
| `01-artefacts/postmortems/` directory with severity threshold "Medium+"     | Incident review process + severity grading      |
| `01-artefacts/task-list/` directory                                         | Sprint planning + closing process               |
| `01-artefacts/impl-plans/` "How we plan to implement X over the next N weeks" | Planning + scoping process                    |
| `01-artefacts/cost-estimates/` cadence                                      | Periodic cost review process                    |
...
4. **Specific bindings selected for first batch**: Scrum Guide 2020 (for `01-artefacts/task-list/`, `01-artefacts/impl-plans/` cadence), BDD (Dan North 2003 / Adzic 2011 for Acceptance Criteria already in `01-artefacts/templates/2_prd.md`), Definition of Done (existing Scrum.org guidance, anchoring the implicit Done state in `WORKFLOW.md` lifecycle), TDD (Beck 2002, for testing rhythm aligned with the `same PR` rule).
...
- [ ] Author `02-process/bdd-workflow.md` opening with a "surfaces Acceptance Criteria already in `01-artefacts/templates/2_prd.md` §FR; binds to Dan North 2003 / Adzic 2011" preamble (Phase 2.1 of impl-plan).
- [ ] Author `02-process/dev-cycle.md` opening with a "surfaces lifecycle implicit in `WORKFLOW.md` Draft→Review→Done; binds to Scrum Guide 2020" preamble (Phase 2.2 of impl-plan).
- [ ] Author `02-process/dod-dor.md` (Phase 2.3) and `02-process/tdd-workflow.md` (Phase 2.4) with parallel preambles.

---

## Compliance / Validation

- Verification: every `02-process/*.md` (excluding the meta-doc) must have a `## 1.5. Surfaces implicit behaviour in <WORKFLOW.md §X | STRATEGY.md §Y>` section.
