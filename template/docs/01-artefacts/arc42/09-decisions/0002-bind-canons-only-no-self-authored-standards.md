...
- **Layer ② Process**: Every `02-process/<canon>.md` file must:
...
- Cons:
  - Some adopters may want a "pentaglyph-flavoured BDD" — they can fork or use downstream `02-process/` extensions, but not the upstream kit.
...
- Downstream projects can replace the canon binding without touching pentaglyph's other layers (e.g. swap Scrum→Kanban by replacing `02-process/dev-cycle.md` only).
...
- Useful practices without an external canon (e.g. team-specific code-review checklists) cannot be upstreamed — they remain in downstream `02-process/`.
...
- [ ] Add a `02-process/_binding-a-new-process.md` meta-doc that documents the 6-section template + paraphrase-detection heuristic (Phase 2 of impl-plan 2026-05-14).
...
- Verification:
  - Every file in `02-process/` matching `^[a-z][a-z0-9-]+\.md$` (i.e. excluding the meta-doc `_binding-*.md`) must contain `## 1. External canon (authoritative)`.
