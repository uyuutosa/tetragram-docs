...
- A `02-process/` file (Layer ②) could prescribe a specific tool (which is Layer ③ Automation's concern).
- A `04-governance/` file (Layer ④) could take a specific decision (which is an individual ADR's role inside Layer ① Artefacts).
- A `05-measurement/` file (Layer ⑤) could prescribe how to improve metrics (which is Layer ② Process's role).
...
(The same table also lives in [`STRATEGY.md §3.2`](../../STRATEGY.md) and [`01-artefacts/arc42/05-building-blocks/pentaglyph-self-architecture.md`](../05-building-blocks/pentaglyph-self-architecture.md). This ADR is the authoritative source; the other two reference it.)
...
- Cons:
  - Edge cases require exception ADRs (e.g. a `scripts/docs/metrics_*.py` reading `04-governance/raci.md` would technically violate ⑤ → ④, but is benign).
...
- [`01-artefacts/arc42/05-building-blocks/pentaglyph-self-architecture.md`](../05-building-blocks/pentaglyph-self-architecture.md) §4 — the dependency-direction diagram.
