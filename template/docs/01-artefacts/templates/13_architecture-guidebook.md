...
> **How to use this template**: `cp docs/01-artefacts/templates/9_architecture-guidebook.md docs/01-artefacts/user-manual/explanation/architecture-guidebook/<chapter>.md`. Delete this `> ...` guidance block before authoring. Split the guidebook into multiple chapter files (one chapter per file) when total length exceeds ~800 lines.
>
> Recommended location: `docs/01-artefacts/user-manual/explanation/architecture-guidebook/` — the long-form variant of the Diátaxis explanation quadrant.
>
...
> - Per-module implementation specs — use [`3_module-detailed-design.md`](./3_module-detailed-design.md) under `01-artefacts/detailed-design/`
> - Decision records (the *Why* of one specific choice) — use [`5_adr.md`](./5_adr.md) (MADR) under `01-artefacts/arc42/09-decisions/`
> - Requirements (WHAT / WHY a feature exists) — use [`2_prd.md`](./2_prd.md)
> - End-user task instructions — use the Diátaxis how-to quadrant under `01-artefacts/user-manual/how-to/`
>
...
|---|---|---|
| `docs/01-artefacts/arc42/` | Authoritative system description (§1–§12) | **Authoritative source**. This guidebook re-narrates arc42 chapters into a continuous read |
| `docs/01-artefacts/diagrams/c4/` | C4 diagrams (Structurizr DSL) | Each chapter links the relevant zoom level |
| `docs/01-artefacts/detailed-design/` | Per-module implementation specs | Linked from each chapter's "Go deeper" footer |
| `docs/01-artefacts/arc42/09-decisions/` (ADR) | Authoritative decision records | Linked inline when each chapter explains *why* |
...
```mermaid
%% Extract the System Context view from docs/01-artefacts/diagrams/c4/workspace.dsl
```

Authoritative source: [`docs/01-artefacts/diagrams/c4/workspace.dsl`](../../01-artefacts/diagrams/c4/workspace.dsl) (Structurizr DSL).
...
- **ADRs**: [ADR-NNNN](../../01-artefacts/arc42/09-decisions/NNNN-*.md), …
- **Detailed designs**: [`docs/01-artefacts/detailed-design/<area>/`](../../01-artefacts/detailed-design/<area>/), …
- **arc42 sections**: [§5 Building Blocks](../../01-artefacts/arc42/05-building-blocks/), [§8 Crosscutting](../../01-artefacts/arc42/08-crosscutting/), …
...
Link to relevant postmortems under `docs/01-artefacts/postmortems/` when available.
...
Convention: [`02-process/code-tours.md`](../02-process/code-tours.md). Authoring discipline for the guidebook itself: [`02-process/architecture-guidebook.md`](../02-process/architecture-guidebook.md).
...
Full ADR index: [`docs/01-artefacts/arc42/09-decisions/README.md`](../../01-artefacts/arc42/09-decisions/README.md).
