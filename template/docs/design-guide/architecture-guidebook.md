...
>
> **What this file is not.** A spec for one module ([`../detailed-design/`](../detailed-design/)). A decision record ([`../arc42/09-decisions/`](../arc42/09-decisions/)). A user manual ([`../user-manual/`](../user-manual/)) — the guidebook lives *inside* `user-manual/explanation/architecture-guidebook/`, but it follows a separate authoring discipline because its scope is system-wide and its audience is developers, not end users.
...
|---|---|---|
| `arc42/` §1–§12 | What is the system? What constrains it? | Reference-style |
| `diagrams/c4/` | What does it look like at each zoom level? | Diagrams |
| `arc42/09-decisions/` (ADR) | Why was each choice made? | Per-decision |
| `detailed-design/` | How is each module implemented? | Per-module |
...
```
docs/user-manual/explanation/architecture-guidebook/
...
3. **Link, do not duplicate.** When a concept is already documented in `arc42/`, `detailed-design/`, or an ADR, link to it — do not re-explain. Re-explanation creates a second canonical location that decays.
...
7. **No diagrams duplicated from `diagrams/c4/`.** Either embed (Mermaid, inlined) **with** the explicit note that `workspace.dsl` is authoritative, or simply link to the SVG render in `docs/diagrams/c4/exports/`. Never paste a hand-drawn diagram that diverges from the DSL.
...
`design-guide/code-tours.md` covers **scenario-specific, execution-order** walkthroughs of a single change or feature. The Architecture Guidebook covers **the whole system**.
...
2. Draft each chapter using [`templates/13_architecture-guidebook.md`](../templates/13_architecture-guidebook.md). Each file = one chapter.
...
- ❌ **The "architecture overview" that is just C4 diagrams with captions.** That is `arc42/05-building-blocks/` already. The guidebook adds narrative voice and the *Why*.
...
- ❌ **The "fifth quadrant" guidebook.** Placing it outside `user-manual/explanation/` because "it's not really explanation". It is. The size is what differs, not the kind.
...
- [`../templates/13_architecture-guidebook.md`](../templates/13_architecture-guidebook.md) — The chapter template
