...
## 1.5. Surfaces implicit behaviour in 01-artefacts/templates/2_prd.md + 01-artefacts/templates/4_use-case.md

pentaglyph has used G/W/T in artefacts since v0.1 without explicitly naming the canon:

- [`01-artefacts/templates/2_prd.md`](../01-artefacts/templates/2_prd.md) — the FR (Functional Requirement) table already requires an "Acceptance Criteria (Given/When/Then)" column.
- [`01-artefacts/templates/4_use-case.md`](../01-artefacts/templates/4_use-case.md) §7 — Acceptance Criteria already uses G/W/T.

This binding makes the canon explicit and gives downstream projects a citable source for the format. Per [ADR-0005](../01-artefacts/arc42/09-decisions/0005-surface-implicit-process-layer.md), this is **surfacing, not inventing**.
...
- **Layer ① templates that consume this binding**:
  - [`01-artefacts/templates/2_prd.md`](../01-artefacts/templates/2_prd.md) — FR table column "Acceptance Criteria (Given/When/Then)"
  - [`01-artefacts/templates/4_use-case.md`](../01-artefacts/templates/4_use-case.md) — §7 Acceptance Criteria
...
- When code lands implementing an FR, the FR's Status moves to `Implemented`, tracked in the corresponding [`01-artefacts/detailed-design/<module>.md`](../01-artefacts/detailed-design/) §"Functional Requirements implemented" section (downstream projects).
...
1. Author `<your-project>/docs/02-process/bdd-workflow.md` that supersedes this file (cite the kit version).
2. Update local copies of `01-artefacts/templates/2_prd.md` and `01-artefacts/templates/4_use-case.md` AC sections.
3. Cite [ADR-0001](../01-artefacts/arc42/09-decisions/0001-adopt-five-layer-self-architecture.md) §5 "Override paths" in the change rationale.
