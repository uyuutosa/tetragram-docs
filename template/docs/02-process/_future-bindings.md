...
> **Layer ② Process — backlog of canons under consideration.** Each entry below has been screened against the [§9.1 four-axis criterion](../01-artefacts/arc42/09-decisions/0003-apply-day1-switching-cost-canon-criterion.md) but **not yet bound**. Promotion to a `<canon>.md` design-guide requires (a) all four axes ✅ and (b) PO approval per [ADR-0002](../01-artefacts/arc42/09-decisions/0002-bind-canons-only-no-self-authored-standards.md).
...
- **Maps to**: would create `05-measurement/dora-baseline.md` (Layer ⑤ Measurement, optional). Pre-requisite: Layer ⑤ activation (Phase 5 of self-architecture roadmap).
...
- **Maps to**: would create `02-process/sre-workflow.md` + bind error-budget / SLO templates. Pre-requisite: arc42 §10 (Quality) population.
...
- **Maps to**: would extend `01-artefacts/templates/6_persona.md` + `01-artefacts/templates/7_journey-map.md` (TiSDD overlap). Touches Layer ① more than pure ② Process.
...
- **Maps to**: would create `01-artefacts/arc42/01-introduction-and-goals/okr-template.md` — touches Layer ① more than ② Process.
- **Status**: Held; current `01-artefacts/arc42/01-introduction-and-goals/overview.md` covers goals informally.
...
- **Maps to**: would create `02-process/lean-startup.md` binding Build-Measure-Learn cycle. Layer ② cadence (alternative to Scrum).
...
- **Why uncertain**: DDD is more an architecture pattern than a process canon — would belong in `01-artefacts/arc42/08-crosscutting/` rather than `02-process/`. Re-evaluate scope.
...
- **Why rejected**: Fails the **external canon** axis by definition. [ADR-0002](../01-artefacts/arc42/09-decisions/0002-bind-canons-only-no-self-authored-standards.md) forbids self-authored process standards.
- **Path**: Author the in-house process in `<downstream>/docs/02-process/` only; do not upstream to pentaglyph.
