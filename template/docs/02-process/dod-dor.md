...
- ADRs `Proposed → Accepted` has its own DoD-equivalent (Y-statement filled, 3+ Drivers, 2+ Options, Consequences in ±/0 form — see [`01-artefacts/arc42/09-decisions/README.md`](../01-artefacts/arc42/09-decisions/README.md)).

This binding makes those implicit gates explicit and aligns them with the Scrum / Scrum.org canon, per [ADR-0005](../01-artefacts/arc42/09-decisions/0005-surface-implicit-process-layer.md).
...
- **DoD checklist template**: [`01-artefacts/templates/11_dod-checklist.md`](../01-artefacts/templates/11_dod-checklist.md) (Phase 2.5 of self-architecture roadmap). Per-artefact-type DoD (PRD-DoD, Module-DoD, ADR-DoD).
- **DoR checklist** is embedded in [`01-artefacts/templates/10_refinement-pbi.md`](../01-artefacts/templates/10_refinement-pbi.md) §"DoR".
...
- **DoD updates**: the team's DoD is itself a Layer A durable artefact (lives in `<downstream>/docs/02-process/dod-checklist.md`), versioned, and tightened over time per Scrum retrospective output.
...
| --- | --- | --- |
| **No DoR** | Trunk-based / continuous-discovery teams that prefer pulling work to refinement | Drop `01-artefacts/templates/10_refinement-pbi.md`. Keep DoD only. |
...
To override: author `<downstream>/docs/02-process/dod-dor.md` and update Layer ① templates 10 / 11 accordingly.
