...
- [`01-artefacts/task-list/`](../01-artefacts/task-list/) Layer B directory exists for "Sprint-scoped task breakdowns" — this is the Scrum **Sprint Backlog** artefact.
- [`01-artefacts/impl-plans/`](../01-artefacts/impl-plans/) "How we plan to implement X over the next N weeks" — implicit Sprint planning horizon.
- [`01-artefacts/postmortems/`](../01-artefacts/postmortems/) "Medium+ severity only" — implicit incident-review process (orthogonal to Sprint Retrospective but related).
...
This binding does not change pentaglyph's behaviour. It names the canon that has been operating implicitly. Per [ADR-0005](../01-artefacts/arc42/09-decisions/0005-surface-implicit-process-layer.md).
...
- **Layer B output directories** the Scrum events fill:
  - **Sprint Planning** output → [`01-artefacts/task-list/YYYY-MM-DD_<sprint>.md`](../01-artefacts/task-list/)
  - **Sprint Retrospective** output → uses [`01-artefacts/templates/9_sprint-retro.md`](../01-artefacts/templates/9_sprint-retro.md) (Phase 2.5 of self-architecture roadmap)
  - **Product Backlog Refinement** output → uses [`01-artefacts/templates/10_refinement-pbi.md`](../01-artefacts/templates/10_refinement-pbi.md)
- **Definition of Done** → [`02-process/dod-dor.md`](./dod-dor.md) (separate binding)
...
| --- | --- | --- |
| **Kanban** (no Sprints, WIP limits) | Continuous-delivery teams, ops-heavy work | Replace this file with `dev-cycle.md` binding [Kanban Method](https://kanban.university/). Drop `01-artefacts/templates/9_sprint-retro.md`; replace with continuous retros. |
...
To override: author `<downstream>/docs/02-process/dev-cycle.md` and update [`STRATEGY.md §9`](../STRATEGY.md) in the same PR.
