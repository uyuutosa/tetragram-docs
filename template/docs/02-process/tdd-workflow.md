...
This binding names the canon that operates implicitly inside every well-formed pentaglyph PR. Per [ADR-0005](../01-artefacts/arc42/09-decisions/0005-surface-implicit-process-layer.md).
...
- **Layer ① test-related artefacts**:
  ...
  - **Test plans** (high-level) → [`01-artefacts/templates/3_module-detailed-design.md`](../01-artefacts/templates/3_module-detailed-design.md) §"Testing strategy" (downstream uses this).
...
| --- | --- | --- |
| **Test-after** | Prototyping / spike work / throwaway code | Document the exception in `<downstream>/docs/02-process/tdd-workflow.md` "Exemptions" section. |
...
To override: author `<downstream>/docs/02-process/tdd-workflow.md` and document the exception class.
