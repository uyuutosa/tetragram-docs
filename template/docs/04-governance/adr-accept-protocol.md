...
> **Layer ④ Governance.** Defines the bar an ADR must clear to leave `Proposed` and become `Accepted` (and therefore immutable per [`01-artefacts/arc42/09-decisions/README.md`](../01-artefacts/arc42/09-decisions/README.md)).

## Why this protocol matters

Per [`01-artefacts/arc42/09-decisions/README.md`](../01-artefacts/arc42/09-decisions/README.md), an ADR's body becomes **immutable once Accepted**. After that point, the decision can only be changed by a *new* ADR that supersedes it. Therefore the bar for `Accepted` is meaningful — it is the point at which the kit (or the project) commits to a decision.
...
- [ ] **The §9.1 four-axis criterion** ([ADR-0003](../01-artefacts/arc42/09-decisions/0003-apply-day1-switching-cost-canon-criterion.md)) is explicitly evaluated for any new operational default proposal.
...
| Acceptor unavailable | Project defines an alternate Acceptor in `<downstream>/docs/04-governance/raci.md`. |
...
`Rejected` ADRs are kept in `01-artefacts/arc42/09-decisions/` for traceability — future readers benefit from knowing what was considered and not chosen.
...
A downstream project may strengthen or relax this protocol by writing `<downstream>/docs/04-governance/adr-accept-protocol.md`. Common overrides:
...
- [`01-artefacts/arc42/09-decisions/README.md`](../01-artefacts/arc42/09-decisions/README.md) — MADR v3.0 file format and authoring rules.
...
- [ADR-0006](../01-artefacts/arc42/09-decisions/0006-self-adr-strict-madr-discipline.md) — kit-level discipline this protocol enforces.
