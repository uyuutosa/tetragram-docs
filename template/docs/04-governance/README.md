...
# 04-governance/ — Layer ④ Governance

> **Self-architecture role**: this directory is **Layer ④ Governance** in pentaglyph's [self-architecture](../01-artefacts/arc42/05-building-blocks/pentaglyph-self-architecture.md). It defines **who decides, accepts, and overrides** — not **what** is decided (decisions are individual ADRs under [`../01-artefacts/arc42/09-decisions/`](../01-artefacts/arc42/09-decisions/)). See [ADR-0001](../01-artefacts/arc42/09-decisions/0001-adopt-five-layer-self-architecture.md), [ADR-0004](../01-artefacts/arc42/09-decisions/0004-layer-separation-contracts.md), [ADR-0008](../01-artefacts/arc42/09-decisions/0008-governance-layer-contract.md).
...
- **Specific decisions.** Decisions are individual ADRs under `01-artefacts/arc42/09-decisions/`. This directory says *how* a decision is reviewed and accepted, not *what* the decision is.
- **Process bindings.** Operational processes (Scrum / BDD / TDD / etc.) live in `02-process/` as Layer ② Process. This directory says *who has authority* over those processes' adoption/override.
- **Code or automation.** Layer ③ Automation lives in `cli/` / `.claude/` / `scripts/docs/`.
- **Metrics dashboards.** Layer ⑤ Measurement (optional) lives in `05-measurement/`.

## Layer ④ dependency direction

Per [ADR-0004](../01-artefacts/arc42/09-decisions/0004-layer-separation-contracts.md), Layer ④ Governance:
...
Downstream may replace any file in this directory by writing a same-named file in `<downstream>/docs/04-governance/`. The override must include a one-paragraph rationale in `<downstream>/docs/02-process/governance-override.md` citing [ADR-0001](../01-artefacts/arc42/09-decisions/0001-adopt-five-layer-self-architecture.md) §5 "Override paths". Common overrides:
...
- [Self-architecture overview](../01-artefacts/arc42/05-building-blocks/pentaglyph-self-architecture.md)
...
- [Templates §12 Governance decision template](../01-artefacts/templates/12_governance-decision.md)
