...
| --- | --- | --- |
| **New template** (Type 12+) | A scenario-table AC template alternative to G/W/T | `01-artefacts/templates/N_*.md` + entry in `01-artefacts/templates/README.md` |
| **New process binding** (Layer ②) | DORA, SRE, Lean Startup binding from `_future-bindings.md` backlog | `02-process/<canon>.md` per `_binding-a-new-process.md` template |
| **Governance refinement** (Layer ④) | Multi-team RACI variant; stricter ADR Accept protocol | `04-governance/<topic>.md` |
...
| **Self-ADR** (kit-meta decision) | Layer ⑤ Measurement activation, new bind exception | `01-artefacts/arc42/09-decisions/NNNN-*.md` |
...
Per [ADR-0002](../01-artefacts/arc42/09-decisions/0002-bind-canons-only-no-self-authored-standards.md):

- **In-house process standards** (no external canon URL). Keep in downstream `02-process/` only.
- **Project-specific naming / styling**. Keep in downstream `02-process/coding-style.md`.
...
│     (per 02-process/ai-augmented-pr.md)                            │
├──────────────────────────────────────────────────────────────────────┤
│  5. Reviewer checks structural + substantive criteria                │
│     (per 04-governance/adr-accept-protocol.md for ADR-bearing PRs)      │
├──────────────────────────────────────────────────────────────────────┤
...
- [ ] **No canon paraphrasing** ([ADR-0002](../01-artefacts/arc42/09-decisions/0002-bind-canons-only-no-self-authored-standards.md)).
- [ ] **Layer separation** ([ADR-0004](../01-artefacts/arc42/09-decisions/0004-layer-separation-contracts.md)): the contribution does not cross layer boundaries inappropriately.
- [ ] **English by default** ([`STRATEGY.md §7`](../STRATEGY.md) §7.5): contributions to the kit are in English.

### Step 4: Open the PR

Use the AI-augmented PR template ([`02-process/ai-augmented-pr.md`](../02-process/ai-augmented-pr.md)) and fill all required fields. If the contribution adds a new operational default, the PR body must include a four-axis evaluation table.
