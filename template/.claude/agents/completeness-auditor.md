...
```text
Glob: docs/**/*.md
Glob: docs/01-artefacts/diagrams/c4/workspace.dsl
```
...
|----------|---------------|-------------------------------|
| `docs/01-artefacts/arc42/01-introduction-and-goals/overview.md` | file exists | length > 500; no `<placeholder>`; ≥ 5 lines starting with `G-` or `- G` (goals); ≥ 3 lines mentioning roles in a stakeholders table; ≥ 5 lines in a quality-goal table |
| `docs/01-artefacts/arc42/03-context-and-scope/system-context.md` | file exists | length > 500; no `<placeholder>`; contains `C4Context` or `Person(` (Mermaid C4 syntax) |
| ≥ 1 PRD under `docs/01-artefacts/arc42/03-context-and-scope/prds/*.md` | glob matches ≥ 1 | ≥ 1 file contains a well-formed FR ID and ≥ 1 file contains a well-formed NFR ID. Well-formed = matches `^(?:\| \*\*)?(FR\|NFR)-[A-Z]{3,6}-\d{3}\*?\*?` at start of line or table cell — order is `FR-CAT-NNN`, **not** `CAT-FR-NNN`. Malformed IDs like `TRACK-FR-003` or `FR-003-AUTH` must be flagged in `INCONSISTENCIES` even if the file otherwise has a correct ID elsewhere. |
| ≥ 1 use case under `docs/01-artefacts/arc42/03-context-and-scope/use-cases/*.md` | glob matches ≥ 1 | ≥ 1 file contains "**Given**" + "**When**" + "**Then**" |
...
|----------|----------|-----------|
| `docs/01-artefacts/arc42/04-solution-strategy/strategy.md` | exists | ≥ 5 markdown table rows that link to `09-decisions/` files |
| `docs/01-artefacts/arc42/05-building-blocks/overview.md` | exists | length > 500; ≥ 3 distinct container names mentioned |
| `docs/01-artefacts/diagrams/c4/workspace.dsl` | exists | contains ≥ 1 `container "..."` declaration that is *not* `"Web App"`, `"API"`, or `"Datastore"` (the placeholders) |
| ≥ 3 ADRs in `docs/01-artefacts/arc42/09-decisions/NNNN-*.md` | glob matches ≥ 3 (excluding README.md) | each contains literal text "Y-statement"; each contains ≥ 2 headings matching `^### Option [A-Z]:` (the per-option Pros/Cons sections in MADR template 5_adr.md). Earlier auditor versions matched on `**Option` line-start which the MADR template does NOT produce — the correct anchor is the `### Option A: <name>` headings. |
...
| Artefact | Existence | Substance |
|----------|----------|-----------|
| ≥ 1 `docs/01-artefacts/detailed-design/<module>.md` | glob matches ≥ 1 (excluding README.md) | length > 2000; contains "## 4.2" (data model) AND "## 4.3" (API spec) AND "## 5" (alternatives) |
| ≥ 1 runtime scenario in `docs/01-artefacts/arc42/06-runtime/<NN>-*.md` | glob matches ≥ 1 (excluding README.md) | contains "sequenceDiagram" or numbered Main Success Scenario table |
| ≥ 2 crosscutting concerns in `docs/01-artefacts/arc42/08-crosscutting/*.md` | glob matches ≥ 2 (excluding README.md) | each contains "Rule:" AND "Why:" |

### Phase 4 — Operations (worth 15%, cumulative target 100%)

| Artefact | Existence | Substance |
|----------|----------|-----------|
| `docs/01-artefacts/arc42/07-deployment/deployment.md` | exists | length > 500 |
| `docs/01-artefacts/arc42/10-quality/slos.md` | exists | ≥ 3 lines containing both "target" and a numeric value (regex `\d+(\.\d+)?\s*(%|ms|s|RPS|req/s|min)`) |
| `docs/01-artefacts/arc42/11-risks/risk-register.md` | exists | ≥ 5 markdown table rows after the header |
| `docs/01-artefacts/arc42/12-glossary/glossary.md` | exists | ≥ 5 markdown table rows after the header |
...
`docs/01-artefacts/arc42/03-context-and-scope/prds/` references an HTTP endpoint /
RPC method / external interface, OR if `docs/01-artefacts/api-contract/` contains
content beyond `README.md`, OR if `docs/02-process/` contains content
beyond `README.md`.

| Artefact | Existence | Substance |
|----------|----------|-----------|
| ≥ 1 schema in `docs/01-artefacts/api-contract/{openapi,graphql,mcp,proto}/*.{yaml,json,md,proto}` per public-API container | glob match per container that exposes a public interface | yaml/json must parse; reference at least one FR-* ID in description fields |
| (optional) `docs/02-process/<topic>.md` files | each contains content beyond Template-0 boilerplate | length > 300 chars |

Applicable target = 100% means **every public-API container has a contract
file** (using `01-artefacts/arc42/05-building-blocks/overview.md` as the source of
truth for which containers are public-facing).

### Phase 6 — End-user documentation (Diátaxis, separate track, conditional)

This track covers `docs/01-artefacts/user-manual/` (Diátaxis 4 quadrants). It is
**applicable** only when the project has shipped at least one
user-visible build or has explicitly opted in.

**Applicability heuristic**: applicable if `docs/01-artefacts/user-manual/{tutorials,how-to,reference,explanation}/`
...
| Artefact | Existence | Substance |
|----------|----------|-----------|
| ≥ 1 file in `docs/01-artefacts/user-manual/tutorials/` | glob match | length > 500; numbered steps |
| ≥ 1 file in `docs/01-artefacts/user-manual/how-to/` | glob match | length > 300 |
| ≥ 1 file in `docs/01-artefacts/user-manual/reference/` | glob match | length > 300; structured (table or list) |
| ≥ 1 file in `docs/01-artefacts/user-manual/explanation/` | glob match | length > 500 |
...
IMPL-COMPANION-COVERAGE: <NN>% | N/A   (Phase 5, applicable if any PRD has API behaviour or 01-artefacts/api-contract/design-guide has content)
END-USER-COVERAGE: <NN>% | N/A   (Phase 6, applicable if 01-artefacts/user-manual/ has any non-README content)
...
NEXT-GAP-TEMPLATE: <template number 0–5 from docs/01-artefacts/templates/>
