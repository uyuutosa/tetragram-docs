...
2. **Front-matter on all durable docs** (`arc42/`, `detailed-design/`, `design-guide/`, `api-contract/`, `user-manual/`, `service-design/`, `governance/`, durable PEL files: `client-engagement/CHARTER.md` / `OPERATING-AGREEMENT.md` / `NOW-NEXT-LATER.md` / `raid.md` / `decisions/`).
...
4. **MADR for ADRs.** No homemade ADR formats. Use `docs/templates/5_adr.md`. Client-visible decisions also use MADR, archived from `client-engagement/daci/` (DACI workflow, Template 17) into `client-engagement/decisions/`.
...
- Inventing a new template (the **nineteen** in `docs/templates/` cover every case — six core + three UX research + five process/governance + five PEL).
